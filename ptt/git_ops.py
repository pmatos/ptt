...
def is_github_repo(path: Path, log_path: Path) -> bool:
    ...
    r = proc.run(["git", "-C", str(path), "config", "--get", f"remote.{REMOTE}.url"],
                 log_path=log_path)
    return r.returncode == 0 and "github.com" in r.stdout


def fetch(path: Path, base_branch: str, log_path: Path) -> None:
    r = proc.run(["git", "-C", str(path), "fetch", REMOTE, base_branch],
                 log_path=log_path)
    if r.returncode != 0:
        raise GitError(f"git fetch {REMOTE} {base_branch} failed in {path}: {r.stderr.strip()}")


def add_worktree(repo: Path, dest: Path, branch: str, base_ref: str,
                 log_path: Path) -> None:
    r = proc.run(["git", "-C", str(repo), "worktree", "add", str(dest),
                  "-b", branch, f"{REMOTE}/{base_ref}"], log_path=log_path)
    if r.returncode != 0:
...
def remove_worktree(repo: Path, dest: Path, log_path: Path) -> None:
    ...
    proc.run(["git", "-C", str(repo), "worktree", "remove", "--force", str(dest)],
             log_path=log_path)
