...
def test_validate_missing_password_fails(fake_bin, github_repo, tmp_xdg, tmp_path, monkeypatch):
    write_config(tmp_xdg["config"], github_repo, tmp_path)
    monkeypatch.delenv("PTT_SMTP_PASSWORD", raising=False)
    assert cli.main(["validate"]) != 0


def test_validate_reports_smtp_password_line(fake_bin, github_repo, tmp_xdg, tmp_path, monkeypatch, capsys):
    write_config(tmp_xdg["config"], github_repo, tmp_path)
    monkeypatch.setenv("PTT_SMTP_PASSWORD", "pw")
    cli.main(["validate"])
...
def test_test_email_fails_fast_without_password(tmp_xdg, github_repo, tmp_path, monkeypatch):
    write_config(tmp_xdg["config"], github_repo, tmp_path)
    monkeypatch.delenv("PTT_SMTP_PASSWORD", raising=False)
    sent = {"n": 0}
...
def test_test_email_sends_provider_neutral_body(tmp_xdg, github_repo, tmp_path, monkeypatch):
    write_config(tmp_xdg["config"], github_repo, tmp_path)
    monkeypatch.setenv("PTT_SMTP_PASSWORD", "pw")
    captured = {}
