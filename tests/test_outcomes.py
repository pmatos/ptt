...
ISS_OPEN_POST = snap(issues={6: {"url": "https://github.com/x/issues/6", "state": "OPEN"}})
...
def test_gh_observed_when_claim_says_none():
    r = outcomes.reconcile(claim(m.Action.NONE, status=m.Status.NO_ACTION, url=None),
                           snap(), PR_POST, True, True, 0, False)
    assert r["action"] == m.Action.PR
...
def test_claude_nonzero_no_file_uses_stderr_reason():
    r = outcomes.reconcile(None, snap(), snap(), True, True, 3, False, stderr_tail="boom happened")
    assert r["status"] == m.Status.ERROR
...
def test_issue_closed_confirmed():
    ...
    r = outcomes.reconcile(claim(m.Action.ISSUE_CLOSED), pre, post, True, True, 0, False)
...
def test_gh_snapshot_command_failure_sets_ok_false(monkeypatch, tmp_path):
    monkeypatch.setattr(outcomes.proc, "run",
                        lambda *a, **k: outcomes.proc.Completed(1, "", "fail"))
    snapshot, ok = outcomes.gh_snapshot(tmp_path, tmp_path / "g.log")
    assert ok is False


def test_read_result_file(tmp_path):
    (tmp_path / ".ptt-result.json").write_text(json.dumps(
        {"status": "success", "action": "pr", "url": "u", "title": "t", "summary": "s"}))
