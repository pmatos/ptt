...
def test_fake_bins_run(fake_bin, tmp_path):
    ...
    r = subprocess.run(["gh", "auth", "status"], cwd=tmp_path,
...
def test_github_repo_fixture_is_github_and_fetchable(github_repo):
    ...
    url = subprocess.run(["git", "-C", str(github_repo), "config", "--get", "remote.origin.url"],
                         capture_output=True, text=True).stdout.strip()
    assert "github.com" in url
    # fetch resolves via insteadOf to the local bare repo
    r = subprocess.run(["git", "-C", str(github_repo), "fetch", "origin", "main"],
                       capture_output=True, text=True)
